...
5. When a screen is finished, press "Mark this screen as done" and move on with the arrows. "All screens" at the top lists every screen by journey and shows what is done.
6. Every edit and approval saves to the server automatically; the browser also keeps a local backup. Check for **Saved to server** at the top. When finished, press **Submit review**. **Download backup** is only an emergency fallback.

Tip: a link with `?screen=<id>` opens a specific screen, so you can point Hamis at exactly the screen you mean.
