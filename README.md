...
The reviewer sees one app in the order a customer meets it: sign up, log in, your details and identity check from the redesign (Figma "CC-onboarding-rough"), followed by the current Home because both supplied Figma Home frames are placeholders, then every other current-app screen captured from Android, and finally the new Send money and Gift a card flows. There is no version switch. The current app's legacy login, sign-up and forgot-password captures are hidden; their keys stay in the bundle and are reviewable under Unmapped copy.
